...
- payload 크기 상한(1MB) — 클라이언트가 push 전 오래된 페이지부터(`lastUpdated` 기준) 잘라내어 상한 내로 맞추고, 그래도 초과하면 push를 막는 최종 안전장치(브라우저 `storage.sync`의 페이지 축출 방식과 동일한 패턴). 잘려나간 페이지는 로컬에는 그대로 남고 다음 push 대상에서만 제외된다
