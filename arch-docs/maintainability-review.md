...
**권고.** 검사 범위를 배포 입력 전체로 넓힙니다. `*.html`과 `*.js`만 스캔하는 것으로는 부족합니다. `styles.css`가 이미 루트 자산이므로 확장자로 좁히면 다음 CSS 자산에서 같은 일이 반복됩니다.

1. 세 종류의 경고 분기를 모두 `process.exit(1)`로 바꿉니다. 매니페스트나 `_locales`가 없는 패키지가 성공으로 끝나는 것에는 정당한 사유가 없습니다.
2. 루트 자산의 완전성 검사를 확장자 화이트리스트가 아니라 "목록에 없는 루트 파일이 있으면 실패"로 만듭니다. `.gitignore` 대상과 문서 파일만 예외로 둡니다.
3. 더 나은 방향은 목록을 버리고 패키지를 참조 관계에서 도출하는 것입니다. 매니페스트가 가리키는 것과 HTML이 `<link>` 및 `<script>`로 가리키는 것을 따라가면 목록을 손으로 갱신할 일이 없어집니다.

1번과 2번은 각각 몇 줄이고, 3번은 별도 과제로 둘 만합니다.
...
2. **`deploy.cjs` 배포 입력 검사** — 5-1. 매니페스트와 디렉터리까지 세 경고 분기를 실패로 바꾸고, 목록에 없는 루트 파일을 찾아 실패시킵니다. 확장자로 좁히면 `styles.css` 같은 자산이 빠집니다.
