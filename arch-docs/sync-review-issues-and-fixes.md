...
### 최종 반영 요약
- `notifyTabHighlightsRefresh`를 단일 탭 전송(`tabs[0]`)에서 매칭된 모든 탭 순회 전송으로 변경.
- 탭별 메시지 전송 실패는 개별 무시하고 나머지 탭 전송을 계속하도록 유지.
- E2E 테스트 `e2e-tests/popup.spec.js`에 "동일 URL 다중 탭에서 하이라이트 삭제 시 모든 탭에 즉시 반영" 케이스를 추가해 회귀를 방지.
