...
import { getSyncedSettings, saveSettingsToSync } from './sync-service.js';
...
export async function loadCustomColors() {
  try {
    ...
    try {
      ...
      if (syncedSettings && syncedSettings.customColors) {
        customColors = syncedSettings.customColors;
        await browserAPI.storage.local.set({ customColors });
        debugLog('Loaded custom colors from bookmark sync');
      }
    } catch (e) {
      debugLog('Failed to read bookmark sync settings, falling back to local:', e.message);
    }
